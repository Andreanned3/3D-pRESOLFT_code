...
if __name__ == "__main__":
    ...
    image = DataIO_tools.load_data(r'Test_data\Origami.tif')
    image = image - image.min()
    thresh = image.max() / 3
    image[image < thresh] = 0
    mean_along_rows = image.mean(1)
    mean_along_cols = image.mean(0)
    pf = pattern_finder()
    [offset_hori, offset_vert, opt_per_hori_px, opt_per_vert_px] = pf.find_pattern(image)
    N = len(mean_along_cols)
    x = np.linspace(0, N-1, N)
    plt.figure()
    plt.plot(mean_along_cols)
    plt.plot(np.cos(2*np.pi*(x-offset_hori)/opt_per_hori_px))
